...
def run_dna_rna_tools(
    ...
    Args:
    - DNA or RNA sequences as str and a procedure to perform as the last argument

    Possible procedures:
    - Main procedures: reverse, transcribe, complement, reverse_complement
    - Additional procedures: nucleotide_frequency, gc_content, find_start_codons

    Returns:
    - Union[List[str], Dict[str, Dict[str, int]], str]: sequence or a collection of sequences with paramters
    """
    for j in args[:-1]:
...
def filter_fastq(
    ...
    """
    Filters provided sequences based on its gc composition, length, and average sequence quality

    Args:
    - seqs Dict[str, Tuple[str, str]]: a dictionary consisting of fastq sequents.
        The key is a string, the name of the sequence.
        The value is a tuple of two strings: sequence and quality.
    - gc_bounds Union[Tuple[float, float], float]: the GC interval of the composition (in percent) for filtration
    - length_bounds Union[Tuple[int, int], int]: the length interval for filtering within which the filtered sequences should be
    - quality_threshold int: the threshold value of the average read quality for filtering
